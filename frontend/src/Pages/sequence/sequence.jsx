...
import './sequence.css';
import { Spinner, Button, Select, Option, Slider } from '@material-tailwind/react';
...
const Sequence = () => {
  ...
  const handleGenerateVideo = () => {
    if (selectedPoses.length > 0) {
      ...
      navigate('/generate', { state: { selectedPoses: newVideo.selectedPoses, filters, duration } });
    } else {
  ...
  const renderAppliedFilters = () => {
    ...
    if (appliedFilters.difficulty_level && appliedFilters.difficulty_level !== 'all') {
      appliedFilterElements.push(<p key="difficulty">Difficulty: {appliedFilters.difficulty_level}</p>);
    }
 ...
 return (
  <div className="relative max-w-7xl mx-auto py-8 px-4" style={{ paddingTop: '100px' }}>
    <div className="flex justify-between items-center mb-6">
      ...
      >
        ...
        <span className="ml-2">{isSidebarOpen ? 'Close Filters' : 'Filters'}</span>
      </Button>
    </div>

 {isSidebarOpen && (
  <>
    <div className="fixed-overlay" onClick={() => setIsSidebarOpen(false)}></div>
    <div className="fixed-sidebar p-6">
      ...
      <div className="filter-inputs space-y-4">
        <Select name="age" label="Age Range" value={filters.age} onChange={(value) => handleSelectChange('age', value)}>
        ...
        <Select name="difficulty_level" label="Difficulty Level" value={filters.difficulty_level} onChange={(value) => handleSelectChange('difficulty_level', value)}>
        ...
        <Select name="focus_area" label="Focus Area" value={filters.focus_area} onChange={(value) => handleSelectChange('focus_area', value)}>
        ...
        <div>
          <h2 className="text-lg font-semibold mt-4">Video Duration: {duration} minutes</h2>
          <Slider value={duration} onChange={handleDurationChange} min={1} max={60} step={1} />
        </div>

        <div className="flex space-x-4 mt-4">
          <Button className="bg-blue-500 text-white w-full text-sm py-2 px-3" onClick={handleApplyFilters}>
            Apply Filters
          </Button>
          <Button className="bg-red-500 text-white w-full text-sm py-2 px-3" onClick={handleResetFilters}>
            Reset Filters
          </Button>
        </div>

        <Button className="bg-yellow-500 text-white w-full mt-4" onClick={handleGenerateRandomVideo}>
